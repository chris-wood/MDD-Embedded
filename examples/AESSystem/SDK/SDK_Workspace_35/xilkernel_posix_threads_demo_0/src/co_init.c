...
{
	AESdataStream=co_stream_create("AESdataStream",INT_TYPE(8),STREAMDEPTH);
	AESoutputStream=co_stream_create("AESoutputStream",INT_TYPE(8),STREAMDEPTH);
	AESinputSignal=co_signal_create("AESinputSignal");

	// Setup the process parameters and then create them
	...
	co_process_create("AESProducer",(co_function)AESProducer,
				1,
				&pParam);
	co_process_create("AESConsumer",(co_function)AESConsumer,
				1,
				&cParam);
