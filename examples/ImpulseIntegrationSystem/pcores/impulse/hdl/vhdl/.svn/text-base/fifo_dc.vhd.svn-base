...
--   Converted to use synchronous read in order to use block RAM.
--
--------------------------------------------------------------------------
...
architecture rtl of fifo_sync_dc is
  ...
  signal read_addra : unsigned(ADDRESSWIDTH-1 downto 0);
  signal doa : std_ulogic_vector(DATAWIDTH-1 downto 0);

  -- Read side signals
  signal out_ptr, next_out_ptr , out_ptr_gray, in_ptr_gray1, in_ptr_gray2 : unsigned(ADDRESSWIDTH-1 downto 0);
  signal read_addrb : unsigned(ADDRESSWIDTH-1 downto 0);
  signal dob : std_ulogic_vector(DATAWIDTH-1 downto 0);
begin
  -- Write side
  ...
  doa <= mem(conv_integer(read_addra));
